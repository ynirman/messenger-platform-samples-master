...
function receivedPostback(event) {
    ...
    case 'GET_STARTED_PAYLOAD':
        var messageData = {
            ...
            , message: {
                text: "Hello " + userName + ", how are you? I heard you are coming to Israel, is it true?"
                , quick_replies: [
                    {
                        "content_type": "text"
                        , "title": "Yes"
                        , "payload": "YES_COMING_TO_ISRAEL"
            }
                    , {
                        "content_type": "text"
                        , "title": "No"
                        , "payload": "NO_COMING_TO_ISRAEL"
            }
          ]
            }
        };
    ...
    case 'YES_COMING_TO_ISRAEL':
        ...
        callSendAPI(messageData);
        break;
    case 'NO_COMING_TO_ISRAEL':
        sendTextMessage(senderID, "Sorry, for now we are not supporting other countries");
        break;
    default:
...
function getUserName(appUserId) {
    var res = syncRequest('GET', 'https://graph.facebook.com/v2.6/' + appUserId + '?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=' + PAGE_ACCESS_TOKEN);
    return JSON.parse(res.getBody('utf8')).first_name;
